...
import { Layout, Menu, Dropdown, Tag, Breadcrumb, ConfigProvider, Button, Divider } from 'antd';
...
export default function AdminLayout({ children, breadcrumbs = [] }) {
    ...
    const menuItems = [
        ...
        { key: '/admin/services', icon: <ToolOutlined />, label: <Link href="/admin/services">Services</Link> },
        { key: '/admin/workers', icon: <TeamOutlined />, label: <Link href="/admin/workers">Workers</Link> },
        { key: '/admin/customers', icon: <UserOutlined />, label: <Link href="/admin/customers">Customers</Link> },
        { key: '/admin/orders', icon: <FileTextOutlined />, label: <Link href="/admin/orders">Orders</Link> },
        { key: '/admin/credit-packages', icon: <CreditCardOutlined />, label: <Link href="/admin/credit-packages">Credit Packages</Link> },
        { key: '/admin/transactions', icon: <SwapOutlined />, label: <Link href="/admin/transactions">Transactions</Link> },
        { key: '/admin/escalations', icon: <WarningOutlined />, label: <Link href="/admin/escalations">Escalations</Link> },
        { key: '/admin/reports', icon: <BarChartOutlined />, label: <Link href="/admin/reports">Reports</Link> },
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <Dropdown menu={{ items: profileMenuItems }} placement="bottomRight" trigger={['click']}>
                            <div className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 px-2 py-1 rounded transition">
                                ...
                                <Tag color="blue" className="ml-1 m-0 capitalize border-none">{auth?.user?.role || 'admin'}</Tag>
                            </div>
                        </Dropdown>
                    </div>
                </Header>
